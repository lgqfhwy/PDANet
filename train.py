...
from models.dam_module import DAM_Module, PAM_Module, CAM_Module
...
def group_weight(module, args):
    group_decay = []
    group_no_decay = []
    group_attention_para = []
    ...
    for m in module.modules():
        #print("sum m = ", m)
        #print("sum m parameters = ", len(list(m.parameters())))
        if isinstance(m, nn.Linear):
            #print("com1 m = ", m)
            #print("com1 m parameters = ", len(list(m.parameters())))
            ...
            #print("com2 m = ", m)
            #print("com2 m parameters = ", len(list(m.parameters())))
            group_decay.append(m.weight)
            if m.bias is not None:
                group_no_decay.append(m.bias)
        elif isinstance(m, nn.modules.batchnorm._BatchNorm):
            #print("com3 m = ", m)
            #print("com3 m parameters = ", len(list(m.parameters())))
            if m.weight is not None:
                group_no_decay.append(m.weight)
            if m.bias is not None:
                group_no_decay.append(m.bias)
        elif isinstance(m, PAM_Module):
            #print("PAM_Module = ", m)
            #print("m parameters = ", list(m.sa.parameters()))
            group_attention_para.append(m.gamma)
        elif isinstance(m, CAM_Module):
            group_attention_para.append(m.gamma)
            #print("group attention para = ", group_attention_para)
        else:
            #print("error m = ", m)
            pass
            #print("error m parameters = ", len(list(m.parameters())))
    #print("group decay = ", group_decay)
    #print("group nodecay = ", group_no_decay)
    # assert False
    # print("len module = ", len(list(module.parameters())))
    # print("len decay = ", len(group_decay))
    # print("len no decay = ", len(group_no_decay))

    assert len(list(module.parameters())) == len(group_decay) + len(group_no_decay) + len(group_attention_para)
    # print("len module = ", len(list(module.parameters())))
    # print("len decay = ", len(group_decay))
    # print("len no decay = ", len(group_no_decay))
    #assert False
    groups = [dict(params=group_decay), dict(params=group_no_decay, weight_decay=.0), dict(params=group_attention_para)]
    return groups
...
def create_optimizers(nets, args):
    ...
    optimizer_encoder = torch.optim.SGD(
        group_weight(net_encoder, args),
        lr=args.lr_encoder,
        momentum=args.beta1,
        weight_decay=args.weight_decay)
    optimizer_decoder = torch.optim.SGD(
        group_weight(net_decoder, args),
        lr=args.lr_decoder,
        momentum=args.beta1,
        weight_decay=args.weight_decay)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch_decoder', default='pdam',
    ...
    parser.add_argument('--root_dataset',
                        default='/home/lgq/Documents/database/')
